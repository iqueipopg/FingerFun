...
def localizeExtremumViaQuadraticFit(
    ...
    for attempt_index in range(num_attempts_until_convergence):
        ...
        first_image, second_image, third_image = dog_images[
        ...
        if (
            i < 1
            or i >= image_shape[0] - 1
            or j < 1
            or j >= image_shape[1] - 1
            or image_index < 1
            or image_index > (num_intervals - 2)
        ):
    ...
    if abs(functionValueAtUpdatedExtremum) * (num_intervals - 2) >= 0.04:
        ...
        if (
            ...
            keypoint.size = sigma * (
                2
                ** ((image_index + extremum_update[2]) / np.float32(num_intervals - 2))
            )
...
def computeKeypointsWithOrientations(
    ...
    scale = (
        scale_factor * keypoint.size
    )  # / float32(2 ** (octave_index + 1))  # compare with keypoint.size computation in localizeExtremumViaQuadraticFit()
            ...
            for col in range(-half_width, half_width + 1):
                ...
                if (
                    ...
                    if (
                        ...
                        dx = (
                            gaussian_images[0][window_row, window_col + 1]
                            - gaussian_images[0][window_row, window_col - 1]
                        )
                        dy = (
                            gaussian_images[0][window_row - 1, window_col]
                            - gaussian_images[0][window_row + 1, window_col]
                        )
                        gradient_magnitude = np.sqrt(dx * dx + dy * dy)
...
def build_vocabulary(
    ...
    bow.build_vocabulary(
        dataset, feature_type=feature_type, vocabulary_size=size, iterations=iterations
    )
